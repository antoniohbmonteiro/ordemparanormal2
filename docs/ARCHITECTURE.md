...
An Ability may own one optional resource containing only current value and maximum, plus an ordered collection of use forms. Each form has a stable ID, rich-text description, structured cost, and optional minimum Agent level. The Agent Sheet does not aggregate the resource; its Ability card exposes a temporary `value/max` summary and a structural 0/1/N use summary. The `useAbility` feature resolves the selected form by ID and validates ownership, permissions, level, current cost and balance before updating either Agent PD or the owning Ability's `system.resource.value`; expected failures are typed results.
