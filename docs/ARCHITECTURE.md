...
The reviewed schema-version-1 JSON presets contain only managed configuration rather than raw Scene exports. The Act I basement contains one native v14 Level, 168 Walls, three interactive Tiles, five linked Agent placements and three control-label Drawings. Its control icon resolves to the system-owned `assets/scene-controls/gm-control-button.png`; the deferred opened-furniture Tile remains absent because its texture would derive from official artwork. The Act II basement contains one Level, 145 Walls, two regular doors and five linked Agent placements, with no Tiles or Drawings. Each map and Token image resolves through semantic adventure assets from the current materialization.
