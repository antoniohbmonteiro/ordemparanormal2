...
The same world-import action continues from Actors to Scenes. `AdventureDefinition.scenes` holds explicit preset references; the current catalog contains only `actOne.basement`. Eligibility comes exclusively from this run's materialization scope. A complete preserved Actor can satisfy a semantic Token binding; Actor cancellation stops the pipeline.

The reviewed schema-version-1 JSON preset contains configuration, one native v14 Level, 168 Walls, three interactive Tiles, five linked Agent placements and three control-label Drawings. It is not a raw Scene export. The full basement map and Token images resolve through semantic adventure assets from the current materialization. The control icon resolves to the existing system-owned `assets/scene-controls/gm-control-button.png`. The fourth furniture Tile is deliberately deferred: its authoring texture is a crop of official Act II artwork and is neither bundled nor derived. The bookshelf control has empty `tileIds` in revision 1 while preserving its two door targets. Future revisions can add that Tile and reference incrementally, without recreating the Scene.
