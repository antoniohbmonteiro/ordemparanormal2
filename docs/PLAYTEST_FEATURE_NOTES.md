...
Investigação é uma feature ativa, construída sobre World Items, Scenes, Regions e o Check Engine existente. **World `pointOfInterest` Item = entidade concreta da campanha; compendium/preset = fonte/template.** O fluxo de Investigação não usa Item embutido em Actor. `name`/`img` são nativos; `system.publicDescription`, `system.gmContext` e `system.information[]` são conteúdo autoral. Cada informação tem `id` estável, `content` e uma ou mais abordagens com perícia, DT e visibilidade da DT. Aptidão exige especialização canônica. Grupos visuais de perícia são derivados das abordagens; descoberta e conhecimento pertencem ao ID único da informação. O Adventure Importer reconcilia conteúdo autoral e ignora flags de runtime no digest.
...
Migration 3 mantém `skills` apenas como campo legado opcional de leitura no DataModel. O pre-flight global valida todos os POIs de World, Actor e ActorDelta de token desvinculado antes de qualquer escrita; se algum for inseguro, informa todos os documentos afetados e não avança a versão mundial. Aptidão legada recebe automaticamente a primeira especialização canônica do registro de perícias (atualmente Artes) e não bloqueia a migração. Após aprovação integral, Apply substitui o `system` autoral, remove `skills`, verifica persistência, consolida os quatro pares conhecidos de POIs importados intactos, remapeia knowledge e atualiza a baseline. POI importado editado manualmente recebe apenas conversão mecânica; ao escolher restaurar depois, os aliases conhecidos são remapeados antes da troca. A compatibilidade de leitura será removida quando não houver mais suporte a worlds anteriores à Migration 3 ou quando outro upgrade puder ler a fonte antiga com segurança.

Continuam fora de escopo: Investigation Item, Note, requirements genéricos, migração do runtime antigo das Regions, resolução automática de pistas, custos de PD, crítico de investigação e novas regras. O julgamento narrativo permanece com o GM.
