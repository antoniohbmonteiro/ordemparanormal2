...
import { ADVENTURE_POI_FLAG_PATH, type PoiImportFlag, type PoiItemPort, type PoiItemSnapshot } from "../../features/adventure-import/import-adventure-pois";
...
function snapshot(item: foundry.documents.Item): PoiItemSnapshot {
  ...
  return { id: item.id!, type: item.type, folderId: item.folder?.id ?? null,
...
export function createAdventurePoiItemPort(): PoiItemPort {
  ...
  function validate(preset: AdventurePoiPreset, flag: PoiImportFlag): void {
    const candidate = new foundry.documents.Item.implementation({ name: preset.name, type: "pointOfInterest", img: DEFAULT_IMG,
  ...
  return {
    ...
    async createItem(preset, folderId, flag, placement: AdventureFolderPlacementFlag) {
      guard();
      const item = await foundry.documents.Item.implementation.create({ name: preset.name, type: "pointOfInterest", img: DEFAULT_IMG,
      ...
      if (!item?.id || item.folder?.id !== folderId || stableSerialize(item.getFlag(SCOPE, "adventureImport")) !== stableSerialize(flag)
