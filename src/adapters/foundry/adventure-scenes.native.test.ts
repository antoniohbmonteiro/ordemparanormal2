...
describe.skipIf(!commonPath)("installed Foundry v14 Scene models (no persistence)", () => {
  it("validates the complete prepared Scene against native document schemas", async () => {
    ...
    const plan = (await prepareAdventureScenes(f.input))[0];
    expect(() => createAdventureScenePort().validateCandidate(plan.desired)).not.toThrow();
