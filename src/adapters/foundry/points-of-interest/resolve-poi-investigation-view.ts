 ...
 * resolved. The returned `view` only ever contains `name`, `description` and
 * `skills` — the field whitelist is enforced by construction.
 */
export async function resolvePoiInvestigationView(
  ...
  const view: PoiInvestigationViewData = {
    ...
    skills: collectPoiInvestigationSkills(item.system).map(skillLabel),
  };
  return { view };
}
