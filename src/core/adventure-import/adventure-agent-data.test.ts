...
describe("Adventure Agent presets", () => {
  it("validates ten complete presets with schema and revision independent", () => {
    expect(PLAYTEST_ALPHA_PRESET_REVISION).toBe(1);
    expect(presets).toHaveLength(10);
    expect(new Set(presets.map(p => p.id)).size).toBe(10);
    expect(presets.filter(p => p.act === "actOne")).toHaveLength(5);
    expect(presets.find(p => p.id === "actOne.eloisa")?.name).toBe("Eloisa");
  ...
  it.each([null, {}, { ...presets[0], level: 11 }, { ...presets[0], level: 1.5 },
    { ...presets[0], schemaVersion: 2 }, { ...presets[0], resources: { healthMax: -1, determinationMax: 0 } },
    { ...presets[0], description: "forbidden" }, { ...presets[0], profile: { ...presets[0].profile, system: {} } },
    { ...presets[0], abilities: [presets[0].abilities[0], presets[0].abilities[0]] },
    ...
    { ...presets[0], abilities: [{ name: "manual", uuid: "Item.manual" }] },
