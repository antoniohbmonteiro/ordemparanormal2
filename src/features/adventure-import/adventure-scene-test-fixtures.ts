...
export function sceneImportFixture(options: { readonly materializedActs?: readonly AdventureAct[] } = {}) {
  const materializedActs = options.materializedActs ?? ["actOne"];
  ...
  const port: AdventureScenePort = {
    ...
    createScene: vi.fn(async (source, folder, placement) => { const id = `scene-${world.length + 1}`; const created = { ...structuredClone(source), _id: id, folder } as unknown as MutableScene; created.flags = { ...created.flags, ordemparanormal2: { ...created.flags?.ordemparanormal2 as object, adventureImportFolder: placement } }; world.push(created); return id; }),
  ...
  const input = {
    ...
    materialization: { materializedActs, assets: PLAYTEST_ALPHA_ADVENTURE.assets.filter(a => materializedActs.includes(a.source.act))
  ...
  return { input, port, actors, world, writes, clearWrites, find,
    scene: (documentId: string) => world.find(candidate => importFlag(candidate)?.documentId === documentId),
    flag: () => importFlag(world[0]), collections: SCENE_COLLECTIONS };
}
