...
class FakePort implements HandoutJournalPort, AdventureFolderPort {
  authorized = true;
  ...
  listFolders(): readonly AdventureFolderSnapshot[] { return this.folders; }
  async createFolder(data: Parameters<AdventureFolderPort["createFolder"]>[0]): Promise<string> {
    const id = `folder-${this.folders.length + 1}`;
    this.folders.push({ id, type: data.documentType, flag: data.flag, name: data.name, color: data.color, parentId: data.parentId });
    return id;
  }
  async updateFolder(id: string, data: Parameters<AdventureFolderPort["updateFolder"]>[1]): Promise<void> {
    const index = this.folders.findIndex(folder => folder.id === id);
    this.folders[index] = { ...this.folders[index], ...data, flag: data.flag as AdventureFolderFlag };
  }
  async createJournal(handout: AdventureDefinition["handouts"][number], storedPath: string, folderId: string, flag: HandoutImportFlag,
    folderPlacement: Parameters<HandoutJournalPort["createJournal"]>[4]): Promise<string> {
    this.journalCreates++;
    if (this.journalCreates === this.failJournalAt) throw new Error("database failed");
    const id = `journal-${this.journals.length + 1}`;
    this.journals.push({ id, flag, folderId, folderPlacement, pages: [{ id: `page-${id}`, flag, type: handout.pageType, src: storedPath }] });
    this.created.push({ label: handout.label, src: storedPath, pageType: handout.pageType });
    return id;
  }
  async updateFolderPlacement(journalId: string, folderId: string | null, folderPlacement: Parameters<HandoutJournalPort["updateFolderPlacement"]>[2]) {
    const index = this.journals.findIndex(journal => journal.id === journalId);
    this.journals[index] = { ...this.journals[index], folderId, folderPlacement };
  }
